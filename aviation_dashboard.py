...
def main():
    ...
    for s in TAF_SITES:
        ss = s.lower()[1:]
        current_tables[f"vis_{ss}"] = to_st_html(v_dfs[s], 'vis')
        current_tables[f"cig_{ss}"] = to_st_html(c_dfs[s], 'cig')
        current_tables[f"llws_{ss}"] = to_st_html(l_dfs[s], 'llws')

    # History Logic
    if os.path.exists(HISTORY_FILE):
        with open(HISTORY_FILE, 'r') as f: full_history = json.load(f)
    else: full_history = []
    full_history.insert(0, {"timestamp": last_updated_str, "tables": current_tables})
